...
#define FKWaterflowViewDefaultCellH 70
#define FKWaterflowViewDefaultMargin 8
#define FKWaterflowViewDefaultNumberOfColumns 3

@interface FKWaterflowView()
/**
 *  所有cell的frame数据
 */
...
@implementation FKWaterflowView
...
- (void)reloadData{
    ...
    // 间距
    CGFloat topM = [self marginForType:FKWaterflowViewMarginTypeTop];
    CGFloat bottomM = [self marginForType:FKWaterflowViewMarginTypeBottom];
    CGFloat leftM = [self marginForType:FKWaterflowViewMarginTypeLeft];
    CGFloat columnM = [self marginForType:FKWaterflowViewMarginTypeColumn];
    CGFloat rowM = [self marginForType:FKWaterflowViewMarginTypeRow];
...
- (void)layoutSubviews{
    ...
    for (int i = 0; i < numberOfCells; i++) {
        ...
        // 优先从字典中取出i位置的cell
        FKWaterflowViewCell *cell = self.displayingCells[@(i)];
...
- (id)dequeueReusableCellWithIdentifier:(NSString *)identifier{
    __block FKWaterflowViewCell *reusableCell = nil;
    [self.reusableCells enumerateObjectsUsingBlock:^(FKWaterflowViewCell *cell, BOOL *stop) {
...
- (CGFloat)cellWidth{
    ...
    CGFloat leftM = [self marginForType:FKWaterflowViewMarginTypeLeft];
    CGFloat rightM = [self marginForType:FKWaterflowViewMarginTypeRight];
    CGFloat columnM = [self marginForType:FKWaterflowViewMarginTypeColumn];
 ...
 *  间距
 */
- (CGFloat)marginForType:(FKWaterflowViewMarginType)type{
    if ([self.delegate respondsToSelector:@selector(waterflowView:marginForType:)]) {
        return [self.delegate waterflowView:self marginForType:type];
    }else{
        return FKWaterflowViewDefaultMargin;
    }
}
...
- (NSUInteger)numberOfColumns{
    if ([self.dataSource respondsToSelector:@selector(numberOfColumnsInWaterflowView:)]) {
        ...
        return  FKWaterflowViewDefaultNumberOfColumns;
    }
}
...
- (CGFloat)heightAtIndex:(NSUInteger)index{
    if ([self.delegate respondsToSelector:@selector(waterflowView:heightAtIndex:)]) {
        ...
        return  FKWaterflowViewDefaultCellH;
    }
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    [self.displayingCells enumerateKeysAndObjectsUsingBlock:^(id key, FKWaterflowViewCell *cell, BOOL *stop) {
